...
func NewOrderItemQueryRepository(client pborderitem.OrderItemServiceClient, opts ...adapter.GuardOption) OrderItemQueryRepository {
	r := &orderItemQueryRepository{client: client}
...
func (r *orderItemQueryRepository) FindOrderItemByOrder(ctx context.Context, order_id int) ([]*models.OrderItem, error) {
	...
	err := r.guard.Call(ctx, func(ctx context.Context) error {
		var callErr error
		resp, callErr = r.client.FindOrderItemByOrder(ctx, &pborderitem.FindByIdOrderItemRequest{Id: int32(order_id)})
		return callErr
	})
	if err != nil {
	...
	for _, item := range resp.Data {
		...
		createdAt := parseTxTime(item.CreatedAt)
		updatedAt := parseTxTime(item.UpdatedAt)
